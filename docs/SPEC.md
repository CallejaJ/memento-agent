...
**Docker mount syntax:** Both read-write and readonly mounts use `-v host:container` syntax. Append `:ro` for readonly: `-v host:container:ro`.
...
1. **Ensures Docker daemon is running** - Exits with a clear error if Docker is not available
